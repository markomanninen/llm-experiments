...
Commands
   ...
   Settings
   ----------------
   - LLM client:        current LLM client (<<llm_client>>)
   - Model:             current LLM model (<<llm_model>>)
   - Stream mode:       conserns only normal LLM requests, not tools, etc. (<<stream_mode>>)
   - Chat mode:         variable only in ollama client between chat/prompt. other clients have only chat mode (<<chat_mode>>)
   - Window size:       last <<window_size>> messages
   - Summary interval   every <<summary_interval>> messages
   - Text-to-speech     current TTS service (<<tts_service>>)
...
You are a JSON generator, you only reply in JSON format. You can generate JSON only according to this schema:
<<response_schema>>
...
- Do not generate validation schema. Gnerate only JSON that validates against the schema.
- Do not repeat function calling tool results given by the system role, for instance:
  Role: system. Tool 0:tool executed with results: {...}.
- Do not generate intros, outros, explanations, or any human language, give only structured JSON data.
- Use tools ONLY when user intents to use them. Do NOT use tools if there is no clear indication that users wants to use them.
- If user asks information about the tools, how to use them, etc. do not activate them, but rather tell, what tools are for.
...
const prompts = {
    ...
    "default": {
        ...
        "agentSystemPrompt": "Your role is to be a specialist, a part of consultants commitee who answers questions related to your field. Answer will be forwarded to a supervisor who collects information given by different specialists and provides the final answer to the user. Give your chain of thoughts in consice format and the final answer as a conclusion.",
        
        "supervisorSystemPrompt":`Your role is to be a supervisor who evaluates consultation given by specialists and provides the final answer to the original user's prompt.

        Original user prompt:
        <<original_prompt>>
        
        Give your reasoning about the original prompt and the answers provided by the agents. Give the final answer to the user based on the agent's input and your best knowledge and evaluation.`,

        "supervisorAgentsPrompt": `<<responses>>`,
        //"secretKeyExposePrompt": secretKeyExposePrompt,
        
        "promptSuggestionPrompt": "Your role is to be a user who inserts a prompt in command line input field which is targeted to an AI assistant in a chat. Respond with the prompt."
    }
}
